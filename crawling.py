...
url_list = [
    ...
    , [2, 'https://chem.dongguk.edu/?page_id=288', '이과대학 통계학과']
...
for list in url_list:
    ...
    # 현재 사이트 type 저장
    now_site_type = list[0]

    # url_list의 loop를 돌면서 url이 변경될 때 마다 현재 페이지를 1로 설정
    curPage = 1

    # url_list의 loop를 돌면서 url이 변경될 때 마다 크롤링 한 게시글의 개수 파악
    site_per = 0
            ...
            if notice == "" or notice is None or notice == "번호":  # 공백인 경우 고정공지이므로 크롤링 하지 않음
                ...
                if link_list[now_site_type] == '':
                    if now_site_type == 1 or now_site_type == 3 or now_site_type == 11:
                        link = url_change + '&' + board.select_one(crawl_var_list[now_site_type][2]).get('href')[17:]
